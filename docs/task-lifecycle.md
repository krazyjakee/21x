...
- **Classes.** `delegated_issue_write` is the only class a Captain performs as ordinary project work. `merge_or_approve`, `deploy_or_release`, `destructive_delete`, `migration_or_replay`, `protection_bypass`, `outbound_message` and `credential_change` each keep their own authorization; `checkIssueWriteCapability` refuses a gated class before it looks at anything else, so "merge this" is never answered with a repository complaint.
