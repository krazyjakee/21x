...
- **Origin.** `resolveIssueWriteOrigin` reads platform records only: a message the user typed in the project chat (the same `latestUserTypedProjectMessage` registry merge grants use), or a Commander relay recorded by `ask_captain` at the moment it ran inside a turn the person started (`recordDelegatedAuthorization`; a report-triggered turn has no stored user message id and records nothing). Tool arguments claiming provenance are ignored and screened. Authorization lapses after `ORIGIN_WINDOW_MS` (one hour). `setIssueWriteOriginResolver` is the seam a durable authorization chain plugs into without the gate changing.
...
- **Exactly once.** `issue_writes` (migration v23, `database/issue-writes-migration.ts`) is both the audit ledger and the idempotency claim, with `idempotency_key` UNIQUE. The key is derived from durable inputs only — project, repository, action, target, task and a hash of the payload — deliberately *not* from who authorized it, so the retry of an interrupted write recomputes the same key across a restart. A created issue also carries `<!-- 21x-issue-write:<key> -->` in its body, which is what makes an external success findable. A claim whose lease expires becomes `unresolved`, never free.
- **Reconciliation.** `reconcileIssueWrites` runs at startup (`recoverIssueWriteOutcomes`) and whenever the ledger is read. For a create it searches the repository for the marker: found means `succeeded` with the real URL, absent means nothing was written and the claim is released. For an update it settles only when the issue already carries the requested title and body. Anything it cannot decide stays `unresolved`, because an unanswerable question is not a negative answer — the same rule `reconcileMergeGrantReservations` follows. `confirmedIssueWriteFailure` distinguishes a refusal GitHub certainly made (HTTP 4xx, `ENOENT`) from an unknown outcome (timeout, reset, kill).
- **Audit.** Each row records the originating human instruction (kind, stored message id, session, a hash of the words and a short excerpt — the words themselves never reach GitHub), the Commander correlation id, the Captain's task and session, the repository, the action and target, the payload hash, the external URL/number and GitHub's answer, the attempt count, and created/updated/settled timestamps. Each settled write also appends a project status journal entry naming the instruction that authorized it. `list_github_issue_writes` reads the ledger and reconciles as a side effect.
