...
| `open_pr` | tell_commander | `open_draft_pull_request` from the signed coding-task scope; draft only, never merge/approve/force/protection |
...
| `issue_write` | tell_commander | `create_github_issue`, `update_github_issue`, `link_github_issue`: delegated work, so the level decides only whether the write is silent, reported or held — never whether it is authorized |
...
A merge under a grant additionally requires an independent approving review of the exact current head, enforced in code. GitHub reports `reviewDecision` `""` on a base branch that requires no reviews, however many people approved, and a branch-level approval may survive a later push when stale-review dismissal is not enabled. Neither is exact-head evidence. `merge_pull_request` therefore reads the latest reviews with their commit OIDs and requires at least one `APPROVED` review by someone other than the PR author whose reviewed commit equals the reread head SHA. Missing, inconsistent or truncated review data fails closed. Without a matching approval it returns `blocked` with `reason_code: INDEPENDENT_REVIEW_REQUIRED` and `needs_external_approval`, reports the blocker to the Commander once per PR head, and spends no grant use. A grant confers authority to merge, never evidence that a PR is safe.
...
A merge grant is the wrong shape for filing a ticket. Merging is a decision only a person may take; opening an issue for work that person just asked for is bookkeeping. Before this distinction existed there was one rule for every external write, so a Captain told "make tasks and matching GitHub issues" could do the tasks and not the issues, and the work stalled. The action taxonomy in `src/shared/issue-actions.ts` splits the two, and `src/main/issue-writes.ts` implements the delegated half.

- **Classes.** `delegated_issue_write` is the only class an ordinary human project-work instruction carries. `merge_or_approve`, `deploy_or_release`, `destructive_delete`, `migration_or_replay`, `protection_bypass`, `outbound_message` and `credential_change` each keep their own authorization; `checkIssueWriteCapability` refuses a gated class before it looks at anything else, so "merge this" is never answered with a repository complaint.
...
- **Origin.** `resolveTaskAuthorization` reads the immutable v23 chain bound to the Captain task by the signed MCP transport. It rechecks expiry, revocation, action permissions, captured repository scope and current project configuration before preflight, after every asynchronous issue/search preflight, before the ledger claim, and immediately adjacent to external dispatch. A changed origin or Captain identity fails closed. The caller task comes from the platform's Captain lookup, never model JSON; report/heartbeat relays and forged provenance fields cannot create a binding.
- **Intent records and diagnostics.** New trusted human ingress stores version-2 classified intents (explicit/necessary basis, source clause range/hash, scope and lifetime) while old version-1 nodes remain unchanged. Refusals name the missing capability, immutable origin node/message, current effective set, failed scope dimension and safe remediation.
...
- **Scope and payload.** The durable authorization repositories are intersected with current project configuration and any repository restrictions on both the calling Captain task and target task, and the full intersection is rebuilt after preflight before a write may dispatch. Update/link performs a GitHub read preflight and refuses pull-request targets. Secret detection scans title, body and every label. Nonempty labels use `gh api` raw nested fields so scalar-looking names such as `true`, `00123` and `null` remain JSON strings on the wire; the valueless typed `labels[]` sentinel represents an explicit empty array.
- **Audit.** Each row records the originating human instruction (kind, stored message id, session, a hash of the words and a short excerpt — the words themselves never reach GitHub), the Commander correlation id, the Captain's task and session, the repository, the action and target, the payload hash, the external URL/number and GitHub's answer, the attempt count, local-effects marker, and created/updated/settled timestamps. Each successful write appends exactly one project status journal entry naming the instruction that authorized it and, when scoped to a task, attaches the issue URL exactly once. `list_github_issue_writes` reads the ledger and reconciles as a side effect.

### Unified ordinary task and PR lifecycle

Trusted human intent is classified once. Task creation, update and start consume
`task.create`, `task.update` and `task.start` from the signed caller's immutable
