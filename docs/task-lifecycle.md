...
- **Exactly once.** `issue_writes` (migration v24, `database/issue-writes-migration.ts`) is both the audit ledger and the idempotency claim, with `idempotency_key` UNIQUE. The key is derived from durable inputs only — project, repository, action, target, task and a hash of the payload — deliberately *not* from who authorized it, so the retry of an interrupted write recomputes the same key across a restart. A created issue also carries `<!-- 21x-issue-write:<key> -->` in its body, which is what makes an external success findable. A claim whose lease expires becomes `unresolved`, never free.
- **Reconciliation.** `reconcileIssueWrites` runs at startup (`recoverIssueWriteOutcomes`) and whenever the ledger is read. For a create it searches the repository for the marker: found means `succeeded` with the real URL, while an absent search result stays `unresolved` because GitHub indexing is not a linearizable negative proof. For an update it hashes only the requested fields recorded in `payload_fields`. Attempt epochs prevent a late writer from settling a row after its lease passed to reconciliation. `confirmedIssueWriteFailure` distinguishes a refusal GitHub certainly made (HTTP 4xx, `ENOENT`) from an unknown outcome (timeout, reset, kill).
