...
Independent review beyond GitHub's configured required reviews, and whether a PR is unsafe, obsolete, duplicate or depends on a predecessor, remain Captain workflow decisions based on current task/repository evidence. The prompt explicitly requires this evidence before each merge and forbids blind retries. The grant parser does not infer these judgments from “safe”, and the merge tool does not claim to implement a dependency or semantic safety oracle.
