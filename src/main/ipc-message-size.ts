...
// Invoke replies (e.g. a full transcript snapshot) are not split, so they get a
// much larger budget that is still far below Chromium's serializer/IPC ceilings.
export const MAX_IPC_REPLY_BYTES = 256 * 1024 * 1024
export const MAX_IPC_REPLY_VALUES = 10_000_000
const MAX_DEPTH = 64
...
export function measureIpcMessage(
  value: unknown,
  limit = MAX_IPC_MESSAGE_BYTES,
  maxValues = MAX_IPC_VALUES
): MessageSize {
  let bytes = 64
  let values = 0
  ...
  const visit = (item: unknown, depth: number): void => {
    if (reason) return
    if (++values > maxValues || depth > MAX_DEPTH) {
      reason = 'complexity'
      return
    }
    add(32)
    ...
    if (item instanceof Error) {
      // Serialized as name/message/stack (+ cause); own props are non-enumerable.
      add((String(item.name).length + String(item.message).length + String(item.stack ?? '').length) * 2)
      if (item.cause !== undefined) visit(item.cause, depth + 1)
      return
    }
    // Class instances are cloned as plain objects of their own enumerable data
    // properties, which is exactly what is measured below. Accessors are still
    // rejected so that measuring never runs user code.
    if (Array.isArray(item) && item.length > maxValues) {
      reason = 'complexity'
      return
    }
