...
describe('OpencodeAdapter', () => {
  ...
  describe('MCP tools survive an OpenCode instance restart', () => {
    ...
    const TASK_MCP = {
      'task-management': { type: 'http' as const, url: 'http://127.0.0.1:53945/mcp?token=launch-token&task=t1&parent=p1' },
    ...
    it('declares the MCP servers in .opencode/opencode.json so a re-created instance rebuilds them', () => {
      ...
      try {
        ...
        expect(written.mcp['task-management']).toEqual({
          type: 'remote',
          url: 'http://127.0.0.1:53945/mcp?token=launch-token&task=t1&parent=p1',
          enabled: true
        })
        ...
        // The URL carries the task API token, so the file must be owner-only.
