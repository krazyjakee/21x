...
import { mkdirSync, writeFileSync, rmSync, existsSync, unlinkSync, readFileSync, chmodSync } from 'fs'
...
export class OpencodeAdapter implements CodingAgentAdapter {
  ...
  private writeRuntimePluginFiles(config: SessionConfig): void {
    ...
    // workspace. We register these files explicitly through the `plugin` key of
    // `.opencode/opencode.json` (see writeWorkspaceOpencodeConfig).
  ...
  private writeWorkspaceOpencodeConfig(openCodeDir: string, config: SessionConfig): void {
    ...
    // workspace untouched.
    if (this.pluginFilePaths.length === 0 && Object.keys(mcp).length === 0 && !existsSync(configPath)) {
      return
    }
