...
`
  ...
  describe('preflightCoversAllChecks', () => {
    ...
    it('treats headings that describe checks as actionable even if they mention status or logs', () => {
      for (const heading of ['## Status checks', '## Monitor error logs', '## Backlog', '## Notebook tasks']) {
        const content = [
          '# Heartbeat Checks',
          '- [ ] Check https://github.com/acme/app/pull/42 for new comments',
          '',
          heading,
          '- [ ] Verify the nightly deploy dashboard is green',
        ].join('\n')
        expect(coversAllChecks(content)).toBe(false)
      }
    })
