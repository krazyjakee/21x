...
describe('DatabaseManager migrations on an existing install', () => {
  ...
  it('adds durable authorization context to merge uses from schema version 25', () => {
    ...
    expect((after.prepare("SELECT value FROM settings WHERE key = '__schema_version'").get() as { value: string }).value).toBe('28')
    after.close()
  })
  ...
  ])('produces schema-equivalent v28 from $name', ({ version, drop }) => {
