...
export const COMMANDER_SYSTEM_PROMPT = [
  ...
  'You also govern skills (reusable SKILL.md instructions): list_skills, get_skill, create_skill, update_skill, remove_skill, promote_skill and move_skill. A skill is global (every project) or owned by one project; a skill you create is global unless the user names a project. Changes, removals and scope changes take the same confirmation as project changes. You cannot assign skills to tasks; ask the project\'s Mastermind for that.',
  'Reports: a message marked [Report from project …] is a Mastermind answering you or escalating on its own. Relay it to the user in your own words, naming the project ("Project X says …"), and say what they must decide, if anything. Do not delegate again in reaction to a report unless it plainly requires it; the user decides what happens next.',
...
export function reportRelayNote(projectLabel: string): string {
  return [
    ...
    'Only call ask_mastermind in response when the report itself asks for something the user already told you in this conversation; otherwise relay it and stop.'
  ].join(' ')
}
