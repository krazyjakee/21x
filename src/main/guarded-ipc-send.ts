  ...
  if (now - lastNoticeAt >= 60_000) {
    ...
    try {
      void Promise.resolve(dialog.showMessageBox({
        type: 'error', title: `${app.getName()} — Update could not be displayed`,
        message: 'An update is too large or too complex to display safely.',
        detail: `${app.getName()} stopped this update to prevent a crash. Saved task data has not been changed. A diagnostic record was saved in logs/ipc-guard.log.`,
        buttons: ['OK']
      })).catch(() => {})
