...
describe('buildAgentSwitchRecap', () => {
  ...
  it('skips generated initial prompts, including earlier handoff recaps', () => {
    const recap = buildAgentSwitchRecap([
      { partId: 'user-initial-1', role: 'user', content: 'Work on task: Add a login form' },
      { partId: 'a1', role: 'assistant', content: 'Added LoginForm.tsx.', partType: 'text' },
      { partId: 'user-initial-2', role: 'user', content: '## Picking up from Agent A\n\nUser: ...' },
      { partId: 'u1', role: 'user', content: 'Now wire it to the API.' },
    ])
    expect(recap).toBe('Previous agent: Added LoginForm.tsx.\n\nUser: Now wire it to the API.')
  })
