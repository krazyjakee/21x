...
describe('IPC size preflight for class instances and errors', () => {
  it('measures class instances and errors like the structured clone does', () => {
    class Info { constructor(public readonly text: string) {} }
    expect(measureIpcMessage(new Info('hello')).reason).toBeUndefined()
    expect(measureIpcMessage(new Info('x'.repeat(MAX_IPC_MESSAGE_BYTES))).reason).toBe('size')
    expect(measureIpcMessage({ error: new Error('boom') }).reason).toBeUndefined()
    expect(measureIpcMessage(new Error('x'.repeat(MAX_IPC_MESSAGE_BYTES))).reason).toBe('size')
  })
})
