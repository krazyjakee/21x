...
export class AgentManager extends EventEmitter {
  ...
  private setupWorktreeIfNeeded(taskId: string): Promise<string | undefined> {
    const { worktreeManager, githubManager, gitlabManager } = this
    return setupTaskWorktrees(this.db, { worktreeManager, githubManager, gitlabManager }, taskId)
  }
  ...
  async notifyParentOfSubtaskCompletion(parentTaskId: string, subtaskId: string): Promise<void> {
    ...
    try {
      const message = buildSubtaskWakeMessage(parentTaskId, subtasks)
