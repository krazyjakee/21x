...
export type PollingHost = Pick<SessionHost,
...
export class SessionPoller {
  ...
  start(sessionId: string, adapter: CodingAgentAdapter, config: SessionConfig, existingSession?: AgentSession): void {
    ...
    adapter.onDataAvailable ??= (dataSessionId: string) => {
      ...
      const entry = this.entries.get(dataSessionId)
  ...
  async pollSingleSession(entry: PollingEntry): Promise<void> {
    ...
    try {
      ...
      if (status.type === SessionStatusType.ERROR) {
        ...
        await this.handleBusyStatus(entry, sessionId, session, batchMessages)
      } else if (status.type === SessionStatusType.IDLE) {
        this.handleIdleStatus(entry, sessionId, session)
      }
    } catch (error: unknown) {
  ...
  private async handleBusyStatus(entry: PollingEntry, sessionId: string, session: AgentSession, batchMessages: OutputMessage[]): Promise<void> {
    const { adapter, config } = entry
    entry.hasSeenWork = true
  ...
  private handleIdleStatus(entry: PollingEntry, sessionId: string, session: AgentSession): void {
    ...
    if (!entry.hasSeenWork && Date.now() - entry.createdAt < IDLE_GRACE_PERIOD_MS) return
    // Some models briefly report idle between tool-call rounds.
    if (entry.lastPartReceivedAt && Date.now() - entry.lastPartReceivedAt < POST_DATA_GRACE_MS) return
    if (this.sendTillDoneNudge(entry, sessionId, session)) return
