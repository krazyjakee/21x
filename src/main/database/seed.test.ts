...
import { LEGACY_COORDINATOR_SKILL_CONTENT, LEGACY_COORDINATOR_SKILL_NAME, seedOrchestratorSkill } from './captain-migration'
import type { DatabaseManager } from '../database'

/** The legacy seeded coordinator skill (pre-Captain name) is retired: untouched copies go, edited ones stay. */
describe('seedOrchestratorSkill migration', () => {
  ...
  function insertSkill(id: string, content: string): void {
    ...
    db.db.prepare(`
      ...
      VALUES (?, ?, 'seeded', ?, 1, 0.8, 0, NULL, '[]', 0, ?, ?)
    `).run(id, LEGACY_COORDINATOR_SKILL_NAME, content, now, now)
  }
  ...
  it('seeds nothing on a new install', () => {
    ...
    expect(db.db.prepare("SELECT id FROM skills WHERE name = ?").all(LEGACY_COORDINATOR_SKILL_NAME)).toEqual([])
    expect(agentSkillIds('agent-a')).toEqual([])
  })

  it('removes an unchanged seeded skill and detaches it from every agent', () => {
    insertSkill('skill-mm', LEGACY_COORDINATOR_SKILL_CONTENT)
