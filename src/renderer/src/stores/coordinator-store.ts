...
/**
 * Which task row hosts each project's Captain (#55).
 *
 * A Captain is a real row in `tasks` (role 'captain', one per project)
 ...
 * and the dashboard talk to the current project's Captain: switching
 * projects switches which id they use, and so which conversation they show.
 */
interface CoordinatorState {
  /**
   * Captain task id by project id. A project is absent until loaded and
   * null when main has no row for it (an unknown project).
   */
  captainTaskIds: Record<string, string | null>
...
export const useCoordinatorStore = create<CoordinatorState>((set, get) => ({
  ...
  load: (projectId) => {
    const known = get().captainTaskIds[projectId]
    ...
    if (!pending) {
      ...
      pending = Promise.resolve()
        ...
        .then((id) => {
          set((state) => ({ captainTaskIds: { ...state.captainTaskIds, [projectId]: id } }))
          return id
        })
        .catch((err) => {
          console.error(`[coordinator-store] Failed to load the Captain task id of project ${projectId}:`, err)
          return null
        })
        .finally(() => {
...
/**
 * The current project's Captain task id, or null until it is known. Loads
 * it on first use and again for every project the user switches to.
 */
export function useCaptainTaskId(): string | null {
  const projectId = useProjectStore((s) => s.currentProjectId)
  const taskId = useCoordinatorStore((s) => s.captainTaskIds[projectId] ?? null)
...
/** A project's Captain task id right now (the current project's by default), for callers outside React. */
export function getCaptainTaskId(projectId: string = getCurrentProjectId()): string | null {
  return useCoordinatorStore.getState().captainTaskIds[projectId] ?? null
}

/**
 * The agent a project's Captain runs on: the project's Captain agent,
 * else its default agent, else the app's default agent (else the first one).
 * An id that no longer names an agent is skipped.
 */
export function captainAgentIdFor(
  project: Pick<ProjectRecord, 'captain_agent_id' | 'default_agent_id'> | undefined,
  ...
  return (
    known(project?.captain_agent_id) ??
