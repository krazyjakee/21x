...
export const CanvasPanel = memo(function CanvasPanel({ panel, zoom, frozen = false, panelIndex, showIndex = false }: CanvasPanelProps) {
  ...
  return (
    <div
      ...
      className={`absolute rounded-xl border bg-[var(--canvas-chrome)] shadow-float flex flex-col transition-shadow duration-150 group/panel ${cfg.border} ${
        isDragging ? 'ring-1 ring-indigo-500/30' : ''
      } ${isConnectingLocal ? 'ring-2 ring-orange-500/50' : ''} ${
        isProximityTarget ? 'ring-2 ring-orange-500/60 shadow-float' : ''
      }`}
      style={panelStyle}
    >
      ...
      {panel.type === 'task' && !isCollapsed && !isDragging && (
        ...
        >
          <button
            ...
            className="flex items-center justify-center w-8 h-8 rounded-full bg-orange-500/20 border border-orange-500/40 hover:bg-orange-500/30 hover:border-orange-500/60 hover:scale-110 transition-all duration-150 cursor-pointer shadow-pop"
      ...
      {showIndex && panelIndex !== undefined && panelIndex < 9 && (
        ...
        >
          <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-indigo-500/90 shadow-float border border-indigo-400/50">
